...
var frontPageSimpleSchedule = {
    ...
    initCalendar : function(month,year){
        $('.sdk_simpleschedule_holder').each(function(){
               var sId = this.id;
               sId = sId.replace('sdk_simpleschedule_holder','');
               
               if($.inArray(frontPageSimpleSchedule.aIdx,sId)==-1){
                   frontPageSimpleSchedule.aIdx.push(sId);
               }
        ...
        var options = {
                ...
                data : {
                    ...
                    iMonth :month,
                    iYear : year,
                    sequence : frontPageSimpleSchedule.aIdx
                    
                },success : function(requestContent){
                    
                    $.each(requestContent.Data,function(index,value){
                          $.each(value.sSchedInfo,function(ind,val){
                              var iSeq = frontPageSimpleSchedule.aIdx[ind];
                              ...
                              calendar ="<p class='sdk_simpleschedule_control'><a href='#' class='sdk_simpleschedule_prev' onclick='frontPageSimpleSchedule.browseMonth("+value.previousMonth+","+value.previousYear+"," + iSeq + ") ; return false' ><img src='/_sdk/img/simpleschedule/blue/scheduler_prev.gif' alt='previous button'/></a><strong class='sdk_simpleschedule_date'>"+value.thisYear+"-"+filterMonth+"</strong><a href='#' class='sdk_simpleschedule_next'  onclick='frontPageSimpleSchedule.browseMonth("+value.nextMonth+","+value.nextYear+"," + iSeq + ") ; return false'><img src='/_sdk/img/simpleschedule/blue/scheduler_next.gif' alt='next' /></a></p>"
                                 ...
                                 if((i - value.startDay + 1)>maxDay){
                                     ...
                                     for(k = 0 ; k<value.sSchedInfo.length ; k++){
                                         if((value.sSchedInfo[k]['schedDate'])==(currentDate+"-"+filterDay)){
                                             totSchedule = "<div class='sdk_simpleschedule_taskcount'><p class='sdk_simpleschedule_count'>"+( (valu.totalSchedule ==0) ? '' : valu.totalSchedule )+"</p></div>"
                                         }
                                     }
                                     
                                    $.each(val,function(inde,valu){
                                          if((valu.schedDate)==(currentDate+"-"+filterDay)){
                                              totSchedule = "<div class='sdk_simpleschedule_taskcount'><p class='sdk_simpleschedule_count'>"+( (valu.totalSchedule ==0) ? '' : valu.totalSchedule )+"</p></div>"
                                          }                                        
                                    })                                     
                                     calendar += "<td align='center' valign='middle' height='20px'><a href='#none' class='"+isToday+"' onclick=\"frontPageSimpleSchedule.viewSchedule('"+currentDate+"-"+filterDay+"'," + iSeq + ") ; return false\">"+day+" "+totSchedule+"</a></td>\n";
                                 }
                             }   
                         ...
                         calendar+="</calendar>";
                         $("#sdk_simpleschedule_output"+iSeq).html(calendar);
        ...
        var options = {
            ...
            data : {
                ...
                $("#sdk_simpleschedule_overlaycontainer"+seq).html(sHtml);
            }
        }
        $.ajax(options);
                  ...
                  $.each(requestContent.Data,function(index,value){
                      sHtml += "<p class='sdk_simpleschedule_taskcontent' id='sdk_simpleschedule_task"+value.idx+"'>";
                          ...
                          if(value.map_location){
                              ...
                              sHtml += "<div id='sdk_scheduleradv_gmap" + seq + "' style='width:230px; height:210px;border:1px solid #CCCCCC;margin:0 auto;'></div>";
                              sHtml += " </div>";
                          }
                      ...
                      if(value.map_location){
                          googleMapApi.viewFrontMap(value.latitude,value.longitude,seq);
                      }
                  });                  
               }
            }
    ...
    },browseMonth : function(month,year,seq){
        ...
        $("#sdk_simpleschedule_overlaycontainer"+seq).empty();
        //$(".sdk_simpleschedule_output").empty();
        
        /** Declare PLUGIN.post data for sending request to the index.**/
        var options = {
                url :usbuilder.getUrl("apiFrontSimpleScheduleCalendarBrowse"),
                type:'post',
                dataType:'json',
                data : {
                    iMonth :month,
                    ...
                    $.each(requestContent.Data,function(index,value){
                        ...
                        calendar ="<p class='sdk_simpleschedule_control'><a href='#' class='sdk_simpleschedule_prev' onclick='frontPageSimpleSchedule.browseMonth("+value.previousMonth+","+value.previousYear+"," + seq + ") ; return false' ><img src='/_sdk/img/simpleschedule/blue/scheduler_prev.gif' alt='previous button'/></a><strong class='sdk_simpleschedule_date'>"+value.thisYear+"-"+filterMonth+"</strong><a href='#' class='sdk_simpleschedule_next'  onclick='frontPageSimpleSchedule.browseMonth("+value.nextMonth+","+value.nextYear+"," + seq + ") ; return false'><img src='/_sdk/img/simpleschedule/blue/scheduler_next.gif' alt='next' /></a></p>"
                        calendar +="<table class='sdk_simpleschedule_days' cellspacing='0' name='day_link'>"                         
                            calendar +="<tr class='sdk_simpleschedule_dotw'>"
                        ...
                        calendar+="</calendar>";
                        $("#sdk_simpleschedule_output"+seq).html(calendar);
                        $(".sdk_simpleschedule_overlay3").remove();
                    });
                }
        }
